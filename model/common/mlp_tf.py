...
class MLP(tf.keras.Model):
    ...
    """
    def __init__(self, dim_list, append_dim=0, append_layers=None, activation_type="Tanh",
                 out_activation_type="Identity", use_layernorm=False, use_layernorm_final=False,
                 dropout=0, use_drop_final=False, verbose=False):
        ...
        for idx in range(num_layers):
            ...
            self.module_list.append(layers.Dense(o_dim))
            ...
            act = activation_dict[out_activation_type] if idx == num_layers - 1 else activation_dict[activation_type]
    ...
    def call(self, inputs, append=None):
        ...
        for layer_idx, layer in enumerate(self.module_list):
            if append is not None and self.append_layers and layer_idx in self.append_layers:
...
class ResidualMLP(tf.keras.Model):
    ...
    """
    def __init__(self, dim_list, activation_type="Mish", out_activation_type="Identity",
                 use_layernorm=False, use_layernorm_final=False, dropout=0):
        super(ResidualMLP, self).__init__()
        hidden_dim = dim_list[1]
        num_hidden_layers = len(dim_list) - 3
        assert num_hidden_layers % 2 == 0, "Number of hidden layers must be even for residual connections."
        ...
        for _ in range(1, num_hidden_layers, 2):
            self.module_list.append(TwoLayerPreActivationResNetLinear(hidden_dim, activation_type, use_layernorm, dropout))
